...
from backend.api.workspaces import get_workspace_internal
...
def _maybe_queue() -> Queue | None:
    ...
    if getattr(client, "is_fake", False):
        return None
    try:
        return Queue(connection=client)  # type: ignore[arg-type]
    except Exception:
        return None
...
async def enqueue(
    ...
    workspace = get_workspace_internal(workspace_id)
    job_id = str(uuid.uuid4())
    set_job_status(job_id, "queued", progress=0.0)
    
    # Use fresh client for job storage
    client = get_client()
    client.hset(f"jobs:{job_id}", mapping={"workspace_id": workspace_id, "provider": workspace.get("provider", "")})
    queue = _maybe_queue()
...
async def status(job_id: str) -> Dict[str, Any]:
    client = get_client()
    data = client.hgetall(f"jobs:{job_id}")
    if not data:
